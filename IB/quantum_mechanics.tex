...
\]
for a particle of mass \(m\) in a potential \(V(x)\) determines states of definite energy and allowed energy values. This is the \emph{time-independent Schrödinger equation}. Solving this equation for hydrogen atom is our eventual aim.
...
Define an infinite potential well
\[
  V(x) =
  \begin{cases}
    0 & |x| \leq a \\
...
\begin{center}
  ...
  \begin{axis}[
    ...
    xlabel=$x$,
    ylabel=$V(x)$]
    \addplot[samples=20,color=blue](-1,x);
    \addplot[samples=20,color=blue](1,x);
  \end{axis}
\end{tikzpicture}
\end{center}
in limit \(u\to\infty\). The time-independent Schrödinger equation is
\begin{align*}
  -\frac{\hbar^2}{2m}\psi'' &= E\psi, |x| \leq a \\
  -\frac{\hbar^2}{2m}\psi'' +u \psi &= E\psi, |x| > a 
\end{align*}
Taking limit \(u\to\infty\), the second equation suggests we must take \(\psi=0\) for \(|x| > a\), whose rationale will be addressed shortly. The appropriate boundary conditions are \(\psi=0\) for \(|x|=a\) and \(\psi\) continuous at \(x = \pm a\). So we are left to solve
\begin{align*}
  -\frac{\hbar^2}{2m} \psi'' &= E\psi, -a\leq x \leq a \\
  \psi(\pm a) &= 0
\end{align*}

For \(E>0\), set \(E=\hbar^2k/2m\) with \(k>0\) and we get
\[
\psi'' + k\psi =0.
\]
So
\begin{align*}
  ...
  &\text{or } A\cos ka = B\sin ka = 0
\end{align*}
so
\begin{align*}
  \text{either } B = 0 &, ka = \frac{n\pi}{2} \text{ for } n = 1,3,\ldots \\
  \text{or } A = 0 &, ka = \frac{n\pi}{2} \text{ for } n = 2,4,\ldots
\end{align*}
Hence
\[
E_n = \frac{\hbar^2\pi^2n}{8ma^2}
\]
for \(n=1,2,3,\ldots\). We obtain a discrete set of allowed energies, with normalised wavefunctions
\[
