...
\begin{defi}
  An $\mathbb{F}$-vector space (or a vector space over $\mathbb{F}$) is an abelian group $(V, +)$ equipped with a function, called \emph{scalar multiplication}:
  \begin{align*}
    \mathbb{F}\times V &\to V \\
...
\begin{lem}[Free functor \(\mathbf{Set} \to \mathbf{Vect}_\F\)]
...
\begin{defi}[Change-of-basis matrix]
...
\begin{defi}[Equivalence of matrices]
...
\begin{defi}[Row and column rank]
  Let \(A\in M_{m,n}(\F)\).
  \begin{itemize}
...
\begin{defi}[Elementary column operation]
...
\begin{defi}[Elementary row operation]
...
\begin{lem}[Dual space as a contravariant functor]
...
\begin{thm}[Double dual as natural transformation]
...
\begin{proof}\leavevmode
  ...
  \item Linear: for \(v_1, v_2 \in V\), \(\lambda_1, \lambda_2 \in \F\), \(\varepsilon \in V*\), then
    \[
      %deal with this
      \widehat{\lambda_1v_1 + \lambda_2v_2(\varepsilon)} = \varepsilon(\lambda_1v_1 + \lambda_2v_2) = \lambda_1 \varepsilon(v_1) + \lambda_2 \varepsilon(v_2) = \lambda_1 \hat v_1(\varepsilon) + \lambda_2 \hat v_2(\epsilon)
    \]
