...
\begin{definition}[Rank \& Nullity]\leavevmode
  \begin{itemize}
...
It follows that
\begin{lemma}
  Let \(V\) be a finite-dimensional \(\F\)-vector space with bases \(\basis E = \{e_1,\dots,e_n\}\) and \(\basis F = \{f_1,\dots,f_n\}\). They have correponding dual bases \(\basis E^* = \{\varepsilon_1,\dots, \varepsilon_n\}\) and \(\basis F^*\). Then the change of basis matrix from \(\basis F^*\) to \(\basis E^*\) is
  \[
    (P^{-1})^T.
  \]
\end{lemma}
...
\subsection{Characteristic Poylynomial of Endormorphism}
...
\begin{proof}\leavevmode
  ...
  \item \(\Leftarrow\): Suppose \(p(\alpha) = 0\) for this \(p\), which is monic wlog. Claim that
    ...
    \begin{proof}
      ...
      \[
        \pi_j \in \ker(\alpha - \lambda_j \iota) = V_{\lambda_j}.
      \]
...
\begin{remark}\leavevmode
  ...
  \item The proof shows that for \(k\) distinct eigenvalues \(\lambda_1, \dots \lambda_k\) of \(\alpha\), the sum \(\sum_j V_{\lambda_j}\) is direct. The only way for diagonalisation to fail is if \(\sum V_{\lambda_j} \lneq V\).
  \end{enumerate}
\end{remark}
...
\begin{theorem}[Simultaneous diagonalisation]
  Let \(\alpha, \beta \in L(V)\) be diagonalisable. Then \(\alpha\) and \(\beta\) are \emph{simultaneous diaognalisable} (there exists a basis with respect to which they are both diagonal) if and only if \(\alpha\) and \(\beta\) commute.
\end{theorem}
...
However, this proof is not satisfactory that it relies on the fact of \(\C\) being algebraically closed which is partially, to say the least, an analytical and not an algebraic property\footnote{Note that while being closed is an algebraic property, the construction of \(\C\) via \(\R\) from \(\Q\) is not.}. In actuality, Cayley-Hamilton is a general result that applies to all fields. Thus we give an alternative algebraic proof:
...
\begin{proof}
  ...
  \[
    B \adj B = \det B I.
  \]
  Let \(B = tI - A\), \(\adj B\) matrix with entries polynomials in \(t\) of degree smaller than \(n\), i.e.\ polynomials in \(t\) with coefficients in \(\M_n(\F)\).
...
\begin{definition}[Algebraic multiplicity]
  Let \(\lambda\) be an eigenvalue of \(\alpha \in L(V)\) where \(V\) is finite-dimensional \(\F\)-vector space. Write
  \[
    \chi_\alpha(t) = (t - \lambda)^{a_\lambda} q(t)
  \]
  for some \(q \in \F[t]\) and \((t - \lambda) \ndivides q(t)\). \(a_\alpha\) is the \emph{algebraic multiplicity} of \(\lambda\) as an eigenvalue of \(\alpha\).
\end{definition}
...
\begin{proof}
  As \(m_\alpha \divides \lambda_\lambda\), \(c_\lambda \leq a_\lambda\).

  As \(\lambda\) is an eigenvalue, \(\alpha(v) = \lambda v\) for some \(v \neq 0\). Now give \(p \in \F[t]\), \(p(\alpha)(v) = p(\lambda)(v)\). Apply this to \(m_\alpha\),
  \[
...
\begin{eg}\leavevmode
  ...
  \item \((t - 2)(t - 1)\).
  \end{itemize}
  We can check that \((A - I)(A - 2I) = 0\) so 2 holds. It follows that \(A\) is diagonalisable.
\item Let
  \[
    A =
    \begin{pmatrix}
...
\begin{proof}\leavevmode
  \begin{itemize}
  \item \(1 \Leftrightarrow 2\): Let \(\lambda_1, \dots, \lambda_k\) be eigenvalues of \(\alpha\). Then \(\alpha\) is diagonalisable if and only if \(V = \bigoplus_i V_{\lambda_k}\). But \(\dim V = \dim n \)
    \begin{align*}
      \dim V &= \dim n = \dim \chi_\alpha = a_1 + \dots + a_k \\
      \dim \bigoplus_i V_{\lambda_k} &= g_1 + \dots + g_k
    \end{align*}
