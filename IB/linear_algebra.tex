...
\begin{eg}
  The bilinear form on \(\R^n\) represented by \(\begin{psmallmatrix} I_p & 0 \\ 0 & 0 \end{psmallmatrix} \in \M_n(\R)\) is positive definite if \(p = n\) and positive semi-definite if \(p < n\).
\end{eg}
...
\begin{remark}
  ...
  for all \(v \in V\).
\end{remark}
which might be closer to the intuitive definition of an ``isometry''.
