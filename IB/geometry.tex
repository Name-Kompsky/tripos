...
\begin{definition}[Smooth embedded surface]\index{Smooth embedded surface}
  \(S \subseteq \R^3\) is a (parameterise) \emph{smooth embedded surface} if for all \(P \in S\), there is an open neighhood \(U = W \cap S\) and a map \(\sigma: V \to U\) from \(V \subseteq \R^2\) such that
  \begin{enumerate}
...
\((u, v) \in V \subseteq \R^2\) are \emph{smooth coordinates} on \(U \subseteq S\). The subspace in \(\R^2\) spanned by\(\frac{\p \sigma}{\p u}(P), \frac{\p \sigma}{\p v}(P)\) is the \emph{tangent space} to \(S\) at \(\sigma(P)\). \(\sigma\) is a \emph{smoth parameterisation} of \(U \subseteq S\).
