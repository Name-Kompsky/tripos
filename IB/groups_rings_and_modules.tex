...
\begin{proof}
  \(N_G(P)\) has index \(n_p\) so \Cref{thm:non-abelian simple group} to get the result. Alternatively, consider the conjugation action of \(G\) on \(\syl_p(G)\).
\end{proof}
...
\begin{definition}[Generated ideal]\index{ideal!generated}
...
\begin{definition}[Free generation]\index{free generation}\index{free module}
...
An \(R\)-module which is freely generated by \(S \subseteq M\) is said to be \emph{free}\index{free module} and \(S\) is called a \emph{basis}\index{basis}.
