...
We will use \nameref{thm:residue} to develop a better understanding of the holomorphic functions.
...
\begin{remark}
  This gives the inverse function theorem: if \(f'(a) \neq 0\), \(\deg_a f = 1\), \nameref{thm:local mapping degree} says that there is a local inverse \(g\) and \nameref{cor:open mapping theorem} implies that \(g\) is continuous. Moreover, \(g\) is holomorphic by the argument in proof of differentiability of power series, which is the same as in IA Analysis I.
\end{remark}
