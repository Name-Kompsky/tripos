...
\begin{theorem}[Cauchy-Riemann equations]\index{Cauchy-Riemann equations}
...
\begin{proof}
  It suffices to prove that the integral along a triangle is zero for all triangles in \(U\). Subdivide a triangle \(T\) into \(4^n\) equal triangles as before. Let \(M = \sup_T |f|\). Let \(T' \subseteq T\) be a subtriangle. If \(T' \cap S = \emptyset\) then the integral is zero by \nameref{thm:weak Cauchy}. Otherwise, for any \(T'\),
  \[
...
\begin{proof}
  ...
  Now consider \(f(z) = \frac{1}{p(z)}\). If \(p\) does not have a zero then \(f\) is an entire function. Moreover, since \(f\) is bounded on \(\{z: |z| \leq R\}\) by continuity and \(|f(z)| < 1\) if \(|z| > R\). By \nameref{thm:Liouville} \(f\) is constant. Absurd.
\end{proof}

\begin{theorem}[Local maximum principle]\index{maximum principle!local}
...
\begin{proof}
  If \(|w - a| < \rho < r\) then by \nameref{thm:Cauchy integral formula}
  \begin{align*}
    f(w) &= \frac{1}{2\pi i} \int_{|z - a| = \rho} \frac{f(z)}{z - w} dz \\
...
\]
Now apply \nameref{thm:Cauchy integral formula} to the first term. Inductively, we derive
\[
  f^{(n)}(w) = \frac{n!}{2\pi i} \int_{|z - a| = \rho} \frac{f(z)}{(z - w)^{n + 1}} dz.
\]
\end{proof}
...
\begin{proof}
  The key here is to use the following characterisation of holomorphicity: a function is holomorphic on a domain if and only if its integral along any closed path vanishes (if given by \nameref{cor:Morera}, only if given by \nameref{thm:weak Cauchy}).
  
  Let \(D = D(a, r) \subseteq U\) be any disc. Then by \nameref{thm:weak Cauchy}, for any closed curve \(\gamma\) in \(D\), \(\int_\gamma f_n(z) dz = 0\). Now \(f_n \to f\) uniformly on \(\gamma\) since \(\gamma([a, b])\) is compact. Since \(f\) is continuous,
  \[
    0 = \int_\gamma f_n(z) dz \to \int_\gamma f(z) dz = 0
  \]
  so by \nameref{cor:Morera} \(f\) is holomorphic on \(D\).

  Next, by \nameref{prop:Cauchy integral formula for derivatives} for any \(w \in D(a, r/2)\),
  \[
    |f'(w) - f_n'(w)|
...
\begin{corollary}[Global maximum principle]\index{maximum principle!global}
...
\begin{theorem}[Casorati-Weierstrass]\index{Casorati-Weierstrass theorem}
...
\begin{eg}
  ...
  \]
  Let \(f(z) = \frac{e^{iz}}{z}\). A naïve choice of semicircular path would pass through the pole at \(z = 0\). Instead consider the semicircular path with a cutout \(\gamma_\varepsilon\) at the origin. This path contains no singularity so has integral zero. By \nameref{lem:Jordan}, \(\int_{\gamma_R} f(z) dz \to 0\) as \(R \to \infty\). Also by the previous lemma,
  \[
...
We will use \nameref{thm:residue} to develop a better understanding of holomorphic functions.
...
\begin{theorem}[Argument principle]\index{argument principle}
...
\begin{proof}
  Apply \nameref{thm:residue} to \(\frac{f'}{f}\). If \(f\) has no zero or pole at \(z\) then \(\frac{f'}{f}\) has no singularity at \(z\). Otherwise apply the previous proposition.
\end{proof}
...
\begin{definition}[Bounded domain]\index{bounded domain}
...
\begin{proof}
  Given the previous discussion, just apply \nameref{thm:argument principle} to \(f(z) - f(a)\). As it has isolated zeros it is non-zero for \(0 < |z - a| \leq r\) if \(r\) is sufficiently small.
\end{proof}

\begin{theorem}[Local mapping degree]\index{local mapping degree}
...
\begin{proof}
  Let \(V \subseteq U\) be open. Need to prove that given \(a \in V\), there is a disc around \(f(a)\) inside \(f(V)\). But by \nameref{thm:local mapping degree}, we can choose \(r, \varepsilon > 0\) such that \(D(a, r) \subseteq V\) and \(D(f(a), \varepsilon) \subseteq f(V)\).
\end{proof}

\begin{remark}
  This gives the inverse function theorem: if \(f'(a) \neq 0\) then \(\deg_a f = 1\), and \nameref{thm:local mapping degree} says that there is a local inverse \(g\) and \nameref{cor:open mapping theorem} implies that \(g\) is continuous. Moreover, \(g\) is holomorphic by the argument in the proof of differentiability of power series, which is the same as in IA Analysis I.
\end{remark}

\begin{theorem}[Rouché]\index{Rouché's theorem}
  \label{thm:Rouche}
...
\begin{eg}
  ...
  \begin{align*}
    ...
    |g| &= |6z + 3| \leq 6|z| + 3 = 15
  \end{align*}
  so \(|f| > |g|\). Thus by \nameref{thm:Rouche}, \(p\) has \(4\) zeros inside \(|z| < 2\). Now consider \(p(z) = \underbrace{z^4 + 3}_g + \underbrace{6z}_f\) on \(|z| = 1\), verify that
  \begin{align*}
    |f| &= |6z| = 6 \\
    |g| &= |z^4 + 3| \leq |z^4| + 3 = 4
  \end{align*}
