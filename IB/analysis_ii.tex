...
\begin{center}
  \begin{tikzpicture}
    \begin{axis}[
      ...
      xlabel=\(x\),
      ylabel=\(f_n(x)\)]
      \addplot[domain=0:.5,thick]{0};
      \addplot[domain=.5:.75,thick]{4*(x-.5)};
      \addplot[domain=.75:1,thick]{1};
    \end{axis}
  \end{tikzpicture}
\end{center}
...
\begin{figure}[h]
  \centering
  \begin{tikzpicture}
    \node [blue] at (2.9,3.4) {\(y = 0\)};
    \node [green] at (4.5,3) {\(y > 0\)};
    \node [red] at (2.9,2) {\(y < 0\)};
    \begin{axis}[
      ticks=none,
      axis equal image,
      axis lines=center,
      ...
      \addplot[domain=2:4,samples=60,green]{sqrt(x^2-4)};
      \addplot[domain=2:4,samples=60,green]{-sqrt(x^2-4)};
      \addplot[domain=-4:-2,samples=60,green]{sqrt(x^2-4)};
      \addplot[domain=-4:-2,samples=60,green]{-sqrt(x^2-4)};

      \addplot[domain=-3.5:3.5,samples=60,red]{sqrt(x^2+4)};
      \addplot[domain=-3.5:3.5,samples=60,red]{-sqrt(x^2+4)};

      \addplot[domain=-.14:.14,very thick,blue]{x};
      \addplot[domain=-.14:.14,very thick,blue]{-x};
      \draw (axis cs:0,0) circle [radius=.2];

      \addplot[domain=-2.79:-1.15,very thick,red]{-sqrt(x^2+4)};
      \draw (axis cs:-2,-2.82) circle[radius=1];
      
      \addplot[domain=2.825:3.18,very thick,green]{sqrt(x^2-4)};
      \draw (axis cs:3,2.23) circle [radius=0.3];
    \end{axis}
  \end{tikzpicture}
  \caption{Level sets for different \(y\)'s}
\end{figure}

For most \(x \in \R^2\), there exists \(\varepsilon > 0\) such that \(B_\varepsilon(x) \cap F^{-1}(F(x))\) is the image of a parameterised curve. But something different happens at the origin: two curves intersect there and there is no neighbourhood of the origin in which it looks like a curve.

\begin{question}
  What is different about \((0, 0)\)?
\end{question}

The answer lies in the derivative of the map. \(\D F|_{(x_1, x_2)} = (2x_1, - 2x_2) \in L(\R^2, \R)\) is surjective everywhere except \((0, 0)\).

\begin{theorem}
  Suppose \(f: \R^n \to \R^m\) is \(C^1\), \(x_0 \in \R^n\) and \(\D f|_{x_0} \in L(\R^n, \R^m)\) is \emph{surjective}. Then there is an open subset \(U \subseteq \R^n\), \(x_0 \in U\) and diffeomorphism \(h: U \to U'\) such that
  \[
...
\begin{proof}
  ...
  If \(v \in \ker \D h|_{x_0}\) then \(v \in \ker L_1 \cap \ker L_2 = 0\) so \(v = 0\). That is to say \(\D h|_{x_0} \in L(\R^n, \R^n)\) is injective and thus invertible. Thus by \nameref{thm:inverse function} there exists \(U \subseteq \R^n\) open, \(x_0 \in U\) such that \(h|_U\) is a diffeomorphism. By construction
  \[
    f(x) = (h_1(x), \dots, h_m(x)).
  \]
\end{proof}
...
We have the tool ready for our theorem. Before that let's introduce some terminologies. Let \(f: \R^n \to \R^m\).

\begin{definition}[Critical point]\index{critical point}
  \(x \in \R^n\) is a \emph{critical point} of \(f\) if \(\D f|_x\) is not surjective.
\end{definition}

\begin{definition}[Critical value]\index{critical value}
...
\begin{theorem}[Global Implicit Function Theorem]
...
\begin{remark}\leavevmode
  ...
  \item Argurably a more realistic physical model would be to have to allow \(\V V\) to have dependence, i.e.
    \[
    ...
    \begin{equation*}
      ...
      \tag{\(\ast\ast\)}
    \end{equation*}
    However, there is a cheap trick that reduces the problem to the previous form. Consider the \(y_0(t), y_1(t), \dots, y_n(t)\) which are components of \(\V y: \R^{n + 1} \to \R^{n + 1}\), satisfying
    \begin{align*}
      y_0'(t) &= 1 \\
...
To solve this type of equation, we will use \nameref{thm:contraction}. Recall that we need two things: a complete metric space \(X\) and a contraction \(F: X \to X\).

Take
\[
  X = \{\V x: [-\varepsilon, \varepsilon] \to \R^n : \V x \text{ is continuous} \}
\]
where \(\varepsilon\) is to be determined. This is a normed vector space with
\[
  \norm{\V x} = \max_{t \in [-\varepsilon, \varepsilon]}\norm{\V x(t)}_2 = \norm*{\norm{\V x(t)}_2}_\infty
\]
We showed back in \Cref{thm:completeness of function space with uniform norm} that \(C[a, b]\) is complete with respect to \(\norm \cdot_\infty\).
...
\begin{proof}
  Suppose \((\V x_k)\) is a Cauchy sequence in \(X\). Write \(\V x_k(t) = (x_{k, 1}(t), \dots, x_{k, n}(t))\). Then for \(1 \leq i \leq n\), \((x_{k, i})\) is Cauchy in \(C[-\varepsilon, \varepsilon]\). Thus there exists \(y_i \in [-\varepsilon, \varepsilon]\) such that \((x_{k, i}) \to y_i\). Then \((\V x_k) \to \V y = (y_1(t), \dots, y_n(t))\) in \(X\) so \(X\) is complete.
\end{proof}
...
\begin{proposition}
  If \(\V x \in X\), \(\V x\) satisfies \eqref{eqn:ODE integral} if and only if \(\V x\) satisfies \eqref{eqn:ODE}.
\end{proposition}

\begin{proof}
  Suppose \(\V x \in X\). Then \(\V V \compose \V x: [-\varepsilon, \varepsilon] \to \R^n\) is continuous so the function
  \[
  ...
  Conversely, if \(\V x'(t) = \V V(\V x(t))\), \(\V x\) is differentiable and thus continuous so \(V x \in X\). Moreover \(\V x'(t) = \V V(\V x(t))\) is continuous (since \(\V V \compose \V x\) is) so \(\V x(t)\) is \(C^1\). Integrating both sides of \eqref{eqn:ODE} gives \eqref{eqn:ODE integral}.
\end{proof}
