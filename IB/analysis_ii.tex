  ...
  \begin{eg}\leavevmode
    ...
    \item Let
      \[
        ...
        \end{cases}
      \]
      Then \(\frac{\partial F}{\partial x_1} = \frac{\partial F}{\partial x_2} = 0\) at \((0, 0)\) but \(F\) is not even continuous. Heuristically, a good definition of differentiability should not allow this as this is not compatible with the definition of differentiability for 1D.
    \item If one insists that differentiability does not imply continuity (by whatever means) and declare \(F\) as differentiable whenever \(\frac{\partial F}{\partial x_i}\) exists, then consider \(G(x_1, x_2) = F(x_1 + x_2, x_1 - x_2)\), whose derivative \(\frac{\partial G}{\partial x_1}\) does not even exist. This is even worse: the composition of differentiable functions is not differentiable!
    \end{itemize}
  \end{eg}
\end{note}
...
\begin{note}
  \(\lim_{t \to 0} f(t x) = w\) for all \( x \in V\setminus \{0\}\) does not imply that \(\lim_{v \to 0} f(v) = w\). This is similar to the case the existence of \(\frac{\partial f}{\partial x_i}\) for all \(i\) does not imply differentiability of \(f\).
\end{note}
...
The analyset way says that give \(f\) in the previous example, the derivative \(\D f: \R^n \to L(\R^n, \R^m), a \mapsto L\) is the constant map.
...
\begin{eg}
  If \(e_i\) is the standard basis vector of \(\R^n\) then \(\D_{e_i} f|a = \D_i F|_a\) if \(\D_i F|_a\) exists.
\end{eg}
...
\begin{corollary}
  If \(f\) is differentiable at \(a\) then \(\D f|_a \in L(\R^n, \R^m)\) is given by multiplicatoin by the \(m \times n\) matrix
\[
  \D f|_a =
  \begin{pmatrix}
    \D_1f_1|_a & \cdots & \D_nf_1|_a \\
...
Now we study some general properties of linear maps.
...
\begin{note}
  The lemma assumes that the vector spaces are finite-dimensional. This does not hold for general normed spaces. For example,
  \begin{align*}
    (C[0, 1], \norm \cdot_1) &\to \R \\
    f &\mapsto f(0)
  \end{align*}
...
\begin{joke}
  I messsed up this proof last year and the next day Trump was elected president.
\end{joke}
...
\filldraw [gray] (3,0) circle (2pt);
  ...
  \]
  Similarly apply MVT to \(g(t) = F(a_1 + h_1, a_2 + t)\): there exists \(y_2(h) = (a_1 + h_1, a_2 + c_2(h))\) such that \(c_2(h) \in [0, h_2]\) and
  \[
