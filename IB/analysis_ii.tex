...
\begin{definition}[Lipschitz equivalence]\index{Lipschitz equivalence}
...
\begin{definition}[Radius of convergence]\index{radius of convergence}
  \[
...
\Cref{prop:pointwise convergence of series} says that \((P_n)\to f\) pointwise on \(B_R(c)\). As \(P_n\) are polynomials so they are continuous. A natural question is, is \(f\) continuous as well? We know this answer will be yes if we can prove that the convergence is uniform.
...
\begin{definition}[Normed subspace]\index{normed subspace}
...
\begin{definition}[\(C^1\) space]\index{Ck@\(C^k\)}
...
\begin{definition}[\(C^2\) space]\index{Ck@\(C^k\)}
...
\begin{proof}[Proof of \nameref{thm:inverse function}]
  ...
  \begin{proof}
    Check the hypotheses for \nameref{thm:contraction}: \(\R^n\) is complete and \(\cl B_\delta(0) \subseteq \R^n\) is closed so \(\cl B_\delta(0)\) is a complete metric space. \nameref{thm:contraction} applied to \(N_y|_{\cl B_\delta(0)}: \cl B_\delta(0) \to \cl B_\delta(0)\) shows that there is a unique \(x \in \cl B_\delta(0)\) with \(N_y(x) = x\), i.e.\ there is a unique \(x \in \cl B_\delta(0)\) with \(f(x) = y\). To remove the bar on top of the ball, apply \(N_y\) again so we get
    \[
...
\begin{definition}[Singular]\index{singular point}
...
\begin{center}
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \addplot[domain=1:2]{((2*x-3)^3-3*(2*x-3)+2)/4};
...
\item After the previous two steps, we have reduced the problem to the form
  ...
  \item and then use \nameref{thm:implicit} to find a function \(G: B_\varepsilon(\V y_0) \to \R^n\) such that
    \[
     \begin{cases} 
       G(\V y_0) = \V z_0 \\
       F(\V y, G(\V y)) = 0
