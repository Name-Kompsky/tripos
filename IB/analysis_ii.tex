...
\begin{remark}\leavevmode
  ...
  \item On \(\R\), if \(f: \R \to \R\) is \(C^1\) and \(f'(x) \neq 0\) then \(f\) is a diffeomorphism onto its image. But this is false for any higher dimension: that \(f: \R^n \to \R^n\) is \(C^1\) with \(\det \D f|_x \neq 0\) does not imply that \(f\) is diffeomorphism onto its image. For example, let
    \begin{align*}
    ...
    \[
      ...
      \end{pmatrix}
    \]
    and \(\det \D f|_{r, \theta} = r \neq 0\) (note that we excluded the origin in the definition, which is always a problem for polar coordinates). But \(f\) is not injective since \(f(r, \theta) = f(r, \theta + 2\pi)\). This shows that the theorem is there is no global generalisation of the one-dimensional case and \nameref{thm:inverse function} is, in a sense, the best alternative we can have about the local generalisation thereof.
  \end{enumerate}
\end{remark}
...
\begin{proof}[Proof of \nameref{thm:inverse function}]
  ...
  \begin{remark}
    ...
    It is now only one step from its final form. We simplify further by approximating \(\D f|_x \approx \D f|_0 = \iota\) where the last step is by our hypothesis. Then we get
    \[
