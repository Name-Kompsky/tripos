...
For most \(x \in \R^2\), there exists \(\varepsilon > 0\) such that \(B_\varepsilon(x) \cap F^{-1}(F(x))\) is the image of a parameterised curve. But something different happens at the origin: two curves intersect there and there is no neighbourhood of the origin such that it looks like a curve.

\begin{question}
  What is different about \((0, 0)\)?
\end{question}

The answer lies in the derivative of the map. \(\D F = (2x_1, - 2x_2) \in L(\R^2, \R)\) is surjective everywhere except \((0, 0)\).
...
The theorem says that there are local coordinates \((y_1, \dots, y_n)\) on \(\R^n\) near \(x_0\) such that \(y_i = h_i(x)\) and with respect ot which \(f\) is the projection of first \(m\) coordinates.
