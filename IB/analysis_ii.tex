...
\begin{proof}
  We first use altenate characterisation of differentiability to rephrase the problem. We know that
  \[
    f(a + h) = f(a) + L_1(h) + \norm h \alpha(h)
  \]
  ...
  So we watnt to find \(\gamma(h)\). Using the estimates for \(f(a + h)\) and \(g(f(a) + r)\), we see that
  \begin{align*}
...
In IA Analysis I, we prove the Mean Value Theorem which is an important result in differential calculus. The results generalise to higher dimension spaces.
