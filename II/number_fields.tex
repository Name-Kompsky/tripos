...
\begin{proof}
  ...
  \[
    B_{r, s}(t) = \left\{ (x, z) \in \R^r \times \C^s: \sum_{i = 1}^r |x_i| + 2 \sum_{i = 1}^s |z_i| \leq t \right\}.
  \]
  ...
  \[
    2^r \left( \frac{\pi}{2} \right)^s \frac{t^n}{n!} = 2^n A(S(I)) = 2^{r + 2s} \n(I) \sqrt{|D_L|}.
  \]
  Rearrange,
  \[
...
\begin{eg}\leavevmode
  Let \(f(x) = x^5 - x + 1\). This is irreducible module \(5\), so over \(\Q\). Let \(L = \Q(\alpha)\) where \(\alpha\) is a root of \(f(x)\). In this case \(r = 1, s = 2\). The discrimiant is
  \[
...
\begin{eg}
  ...
  \[
    a^2 - 10b^2 = \pm 2.
  \]
  Reduce modulo \(5\), either \(2\) or \(-2\) is a quadratic residue. Absurd. Thus \(\mathfrak p_2\) is not principal and
  \[
    \Cl(\O_L) \cong \Z/2\Z.
  \]
\end{eg}
...
The proof the theorem will show \(\ell(\O_L^\times)\) is a lattice in \(H\).
