...
\begin{proof}
  ...
  \begin{align*}
    z_0 &= a_0 + (1 - \zeta_p)(a_1 + (1 - \zeta_p)z_2) \\
        &= a_0 + (1 - \zeta_p)a_1 + (1 - \alpha_p)^2z_2
  \end{align*}
...
What are the units in this ring? There are the obvious one \(pm 1\) but are there any else?
...
\begin{proof}
  ...
  We've seen that \((1 - \zeta_p)^{p - 1} = p\O_K\), so \(p\) is ramified in \(K\). Let \(\ell \neq p\) be a prime. Since \(\Z[\zeta_p] = \O_K\), \nameref{thm:Dedekind} tells us that \(\ell\) is ramified in \(K\) if and only if \(f_p(x) \pmod \ell\) has a repeated root. We know \(\disc f_p = \pm p^{p - 1}\), hence \(\disc(f_p \pmod \ell) \neq 0\) so \(f_p(x) \pmod \ell\) does not have any repeated roots.
\end{proof}
...
\begin{proof}
  \(\{\pm \zeta_p^i: 0 \leq i < p\} \subseteq \mu_K\) is a subgroup of order \(2p\) so it suffices to show \(|\mu_K| = 2p\). If \(\ell \neq p\) is an odd prime and \(\ell \divides |\mu_K|\) then \(\zeta_\ell \in \Q(\zeta_p)\) so \(\Q(\zeta_\ell) \subseteq \Q(\zeta_p)\). In particular \(\ell\) is ramified in \(\O_{\Q(\zeta_\ell)}\), so there exists a prime ideal \(\mathfrak p \subseteq \O_{\Q(\zeta_\ell)}\) such that \(\mathfrak p^2 \divides \ell \O_{\Q(\zeta_\ell)}\). Then \(p^2 \O_K \divides \ell \O_K\) so \(\ell\) is ramified in \(K\), absurd.
\end{proof}
