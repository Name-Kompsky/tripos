...
\begin{definition}[Ramification]\index{ramified}\index{inert}\index{split completely}
...
\begin{proof}
  ...
  \[
    A/pA \cong \Z[x]/(pf(x)) \cong \F_p[x]/(\red f(x)).
  \]
  ...
  Define \(\mathfrak p_i \subseteq \O_L\) to be the ideal containing \(p\) such that \(\mathfrak p_1/(p)\) is the kernel of the ring homomorphism
  \[
    \begin{tikzcd}
    \O_L/p\O_L
    \ar[r, "\red \varphi^{-1}"]
...
\begin{definition}[Covolume]\index{covolume}
  ...
  \[
    A(\Lambda) = \operatorname{vol}\left( \left\{ \sum_{i = 1}^n t_iv_i: t_i \in [0, 1]\right\}\right)
  \]
...
To demonstrate how to actually compute and use covolume, we state a theorem whose general version will be proved later in the course:

\begin{theorem}[Special case of Minkowski's theorem]
  Let \(\Lambda \subseteq \R^2\) be a lattice and let \(S = D(0, r) \subseteq \R^2\) be the closed disc of radius \(r\). Then if \(\operatorname{area}(S) \geq 4A(\Lambda)\) then there exists \(\lambda \in \Lambda \setminus \{0\}\) such that \(\lambda \in S\).
\end{theorem}
