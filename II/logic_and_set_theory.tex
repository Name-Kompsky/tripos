...
In this section we build the universe of sets starting with \(\emptyset\) and taking power set many times.
...
The slogan is ``the universe is the union of sets'', or \(V = \bigcup_{\alpha \in ON} V_\alpha\) where \(ON\) is the class of ordinals. Note the subtlety hidden in the notation: you cannot take union of a family indexed by a proper class! (or even more pedantically, the result is not a set)
...
\begin{remark}\leavevmode
  ...
  \item What the proof says essentially is
    ...
    \]
    which is the right way to think about rank. For example, \(\rank(\{6\})\) is \(7\) as \(\rank(6) = 6\) since it is cardinal.
...
We will discuss ``sizes'' of sets in this chapter. We will work predomiantly under ZFC, although we will not refrain ourselves from comparing and contrasting results if working under ZF.
