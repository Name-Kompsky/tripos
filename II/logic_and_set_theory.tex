...
If \(p\) has free vairables, we can define \(p_A \subseteq A^{\#\text{free variables of \(p\)}}\). For example, if \(p\) is the formula \((\exists y) (m(y, y) = x)\), then
\[
...
\begin{enumerate}
\item modus ponens\index{modus ponens}: from \(p, p \implies q\) can deduce \(q\).
...
\begin{proof}
  The model constructed in \nameref{thm:model existence lemma} of \(\cl S\) is countable.
\end{proof}
...
\begin{definition}[Zermelo-Fraenkel set theory]\index{Zermelo-Fraenkel set theory}\index{ZF}
...
\begin{note}\leavevmode
  ...
  \item ZF axioms do not include axiom of choice. ZF~\(+\)~AC is called ZFC where\index{ZFC}
  \end{enumerate}
\end{note}
...
\begin{theorem}[\(\in\)-recursion]\index{\(\in\)-recursion}
