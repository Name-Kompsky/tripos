...
\renewcommand{\P}{\prob} % probability
...
\begin{definition}
  An incomplete graph \(G\) is \emph{\(k\)-connected} if whenever \(W \subseteq V(G)\) with \(|W| < k\) then \(G - W\) is connected. %? why exclude complete graph
\end{definition}
...
\begin{theorem}[Menger]\index{Menger theorem}
...
\begin{definition}[edge-connectivity]\index{edge-connectivity}
...
\begin{corollary}[edge Menger]\index{Menger theorem!edge}
