...
\begin{eg}
  Let \(f(t) = t^6 + 3\). Let \(\xi = - \omega\) be a primitve \(6\)th root of unity where \(\omega\) is a primitive cubic root of unity. Then
  \[
    \Q(\xi) = \Q(\omega) = \Q(\sqrt{-3}).
  \]
  \(f(t)\) is irreducible over \(\Q\) by Eisenstein with \(3\). However over \(\Q(\xi) = \Q(\sqrt{-3})\). \(f(t)\) factorises as
  \[
    f(t) = (t^3 + \sqrt{-3}) (t^3 - \sqrt{-3})
  \]
  so the splitting field \(L\) of \(f(t)\) over \(\Q(\xi)\) is a proper subgroup of \(\Z/6\Z\).
\end{eg}
\end{document}