...
\begin{proof}
  ...
  \[
    ...
    \end{pmatrix}
  \]
  Let \(L\) be a splitting field of the minimal polynomial \(f(t)\) of \(\alpha\) over \(K\).Then \(f_\alpha(t) = \prod_{i = 1}^n (t - \alpha_i)\) with \(\alpha_1, \dots, \alpha_n \in L\). The entries in \(A\) are of the form \(\Tr_{M/K}(\alpha^L)\) which is \(\alpha_1^l + \dots + \alpha_n^l\) by \Cref{thm:trace and norm of separable extension}.
