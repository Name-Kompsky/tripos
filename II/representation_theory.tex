...
This is a faithful representation of dimension \(|G|\). We call \(V = \F G\) (sometimes also written as \(\F[G]\)) the \emph{regular module}\index{regular module}.
...
\begin{definition}[character]\index{character}\index{character!irreducible}\index{character!faithful, trivial}
...
\begin{theorem}\leavevmode
  \begin{enumerate}
  \item \(\chi_V(1) = \dim V\).
  \item \(\chi_V\) is a \emph{class function}\index{class function}, namely it is conjugation invariant. Thus \(\chi_V\) is constant on conjugacy classes of \(G\).
...
\begin{definition}[character table]\index{character table}
...
Before proving 2, let's prove column orthogonality, assuming \Cref{cor:number of irreducible equals to class number}.
...
\begin{notation}
  ...
  \]
  by \Cref{lem:product permutation character}.
\end{notation}
...
Recall that the \emph{group algebra}\index{group algebra} \(\C G\) of a finite group \(G\), the \(\C\)-space with basis \(G\) and dimension \(|G|\). It is also a ring and a \(\C\)-algebra.

Let \(\{1\} = \ccl_1, \ccl_2, \dots, \ccl_k\) be the \(G\)-conjugacy classes. Define the \emph{class sums}\index{class sum}
\[
...
\begin{definition}[representation of topological group]\index{representation!of topological group}
...
\begin{remark}
  It can be easily seen that these are representations. Why are they the only ones? If one drops continuity condition, the number of 1-dimensional representations is uncountably infinite. See Teleman \textsection 19.8.
\end{remark}
...
\begin{proof}[Proof of \Cref{thm:representation of S1}]
...
\begin{definition}[Haar measure]\index{Haar measure}
...
\begin{corollary}[Weyl's unitary trick]\index{Weyl's unitary trick}
...
\begin{corollary}[Maschke]\index{Maschke's theorem}
...
\begin{theorem}[row orthogonality]\index{row orthogonality}
  ...
  \]
\end{theorem}
Naturally one may wonder if irreducible characters form a basis of \(\mathcal C(G)\). The answer is not quite. We need some Hilbert space theory and Peter-Weyl theorem. For \(S^1\) see Teleman \textsection 19.14, 19.15.
...
\[
  ...
  \right\}
\]
\emph{Hamilton's quaternion algebra}\index{Hamilton's quaternion algebra}. \(\H\) is a 4 dimensional Euclidean space and \(\norm A^2 = \det A\) defines a norm on \(\H \cong \R^4\) with \(G\) the unit ball. If \(A \in G\) and \(X \in \H\) then
\[
  \norm{AX} = \norm{X} = \norm{XA}.
\]
...
\begin{proof}
  If \(V\) is a representation of \(G\) then \(\Res^G_T V\) is a representation of \(T\) and its character \(\chi_{\Res^G_T V}\) is the restriction of \(\chi_V\) to \(T\). But every representation of \(T\) has character of given form by \Cref{thm:representation of S1}.
\end{proof}
...
\begin{remark}
  Alternatively, see Teleman \textsection 21.1 we can evaluate \(\ip{\chi_n, \chi_n} = 1\) using Weyl's integration formula.
\end{remark}
...
We know from \Cref{lem:represntation of SU(2) determined by its restriction to maximal torus} for \(V, W\) representation of \(G\), \(\Res_T^G V \cong \Res_T^G W\) implies \(V \cong W\). We want to understand tensor products of representations for \(G\).
...
\begin{theorem}[Clebsch-Gordon formula]\index{Clebsch-Gordon formula}
  For any \(n, m \in \N_0\),
  \[
...
\begin{proof}[Sketch proof of \Cref{prop:isomorphisms of Lie groups} 1]
