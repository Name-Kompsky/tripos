...
\begin{proof}
  ...
  \[
    \sum_{i = 1}^k \deg \chi_i \ip{\chi_i\downarrow_H, \psi}
    = \ip{\pi_{reg} \downarrow_H, \psi}
    = \frac{|G|}{|H|} \psi(1)
    \neq 0
  \]
  so \(\ip{\chi_i \downarrow_H, \psi} \neq 0\) for some \(i\).
\end{proof}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &= \frac{H}{G} \ip{\chi_H, \chi_h} + \frac{1}{|G|} \sum_{g \in G \setminus H} |\chi(g)|^2 \\
...
\begin{theorem}[Frobenius reciprocity]\index{frobenius reciprocity}
  ...
  \[
    \ip{\varphi_H, \psi} = \ip{\varphi, \psi^G}.
  \]
\end{theorem}
...
\begin{proof}
  \begin{align*}
    \ip{\varphi, \psi^G}
    &= \frac{1}{|G|} \sum_{g \in G} \conj{\varphi(g)} \psi^G(g) \\
