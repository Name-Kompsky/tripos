...
\begin{proof}
  Write \(G = \langle x_1 \rangle \times \dots \times \langle x_r \rangle\) where \(|x_j| = n_j\). Suppose \(\rho\) is irreducible so it is 1 dimensional. Let \(\rho(1, \dots, x_j, \dots, 1) = \lambda_j\). Then \(\lambda_j^{n_j} = 1\) so \(\lambda_j\) is an \(n_j\)th root of unity. Now the values \((\lambda_1, \dots, \lambda_r)\) determine \(\rho\).
\end{proof}
