...
\begin{definition}[equivalent norm]\index{equivalent norm}
...
\begin{proposition}
  Let \(X\) and \(Y\) be normed spaces, \(Y\) complete and let \(T \in B(X, Y)\). Then there is a unique \(\tilde T \in B(\tilde X, \tilde Y)\) such that
  \[
    \tilde T|_X = T, \norm{\tilde T} = \norm T.
  \]
\end{proposition}
...
They are equivalent\index{equivalent norm} for precisely the same reason that norms on finite-dimensional spaces are equivalent. Thus henceforth we will just use ``norm on \(X \times Y\)'' to mean any of the equivalent norms.
...
\begin{definition}[normed/Banach algebra]\index{normed algebra}\index{Banach algebra}\index{algebra!normed}\index{algebra!Banach}
...
\begin{definition}[spectrum, resolvent]\index{spectrum}\index{resolvent}
...
\begin{definition}[point spectrum, continuous spectrum, residual spectrum]\index{spectrum!point}\index{spectrum!continuous}\index{spectrum!residual}\index{eigenvalue}
...
\begin{remark}
  By previous proposition, if \(\lambda \in \sigma_c(T)\) then \(T - \lambda\) is not bounded below. % possibility that it is left but not right inverse?
  Thus there exists a sequence \((x_k) \subseteq X\) with \(\norm{x_k} = 1\) such that \(T x_k - \lambda x_k \to 0\) as \(k \to \infty\). \(\lambda\) is called an \emph{approximate eigenvalue}\index{approximate eigenvalue}.

  The set
  \[
    \sigma_{ap}(T) = \{\lambda \in \sigma(T): \lambda \text{ is an approximate eigenvalue}\}
  \]
  is the \emph{approximate point spectrum}\index{spectrum!approximate point}.
\end{remark}
...
\begin{definition}[dual/adjoint]\index{dual}\index{adjoint}
...
\begin{proof}
  ...
  \[
    \norm{T^*f}_{X^*} \leq \norm f_{Y^*} \norm T
  \]
  so \(\norm{T^*} \leq \norm T\). In general \(\norm{T^*} \geq \norm T\) follows from the Hahn-Banach theorem\index{Hahn-Banach theorem} but for some spaces, such as \(\ell^p\) and Hilbert spaces we may write down an explicit element saturating the bound.
\end{proof}
...
\begin{definition}[normal, self-adjoint, unitary]\index{normal}\index{self-adjoint}\index{unitary}
  \(T \in B(H)\) is
  \begin{itemize}
...
\subsection{Spectral theorem for compact self-adjoint operators}\index{compact operator}
...
\begin{lemma}\index{approximate eigenvalue}\index{eigenvalue}
...
\begin{theorem}[Hilbert-Schmidt]\index{Hilbert-Schmidt theorem}\index{spectral theorem for compact self-adjoint operators}
  ...
  \end{align*}
  where \(P_{\lambda_j}\) is the orthogonal projection\index{orthogonal projection} onto \(E_{\lambda_j}\).
\end{theorem}
...
\begin{corollary}
  Let \(H\) be a separable\index{separable} and \(T \in B(H)\) self-adjoint and compact. Then there is an orthonormal basis for \(H\) of \(T\) eigenfunctions.
\end{corollary}
...
\begin{theorem}
  There exists a continuous function (\emph{Green function}\index{Green function}) \(k: [a, b] \to \R\) such that the unique solution \(u \in C^2[a, b]\) to the bundary value problem
  \begin{align*}
...
\begin{proof}
  ...
  % TODO: check definition of compactness
\end{proof}
...
% This gives inverse of \(T\). A stategy commonly used in PDE: the original operator might not be bounded or compact but its inverse is.
% we can also look at resolvent: in the resolvent set
...
\]
% something something so eigenfunctions as continuous functions???
...
%Physically, the eigenfunctions are the energy eigenfunctions.
