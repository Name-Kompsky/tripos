...
\begin{eg}
  \(f(z) = \frac{1}{e^z - 1}\) is meromorphic on \(\C\) with poles at \(z = 2\pi n i\) where \(n \in \Z\). By considering \(g(z) = \frac{z}{e^z - 1}\) which has removable singularity at \(0\), we know \(f\) has a pole of order \(1\) at \(0\), and therefore at all poles by periodicity.
...
\begin{fact}\leavevmode
  ...
  \item (corollary of classification of compact surfaces) every compact Riemann surface is homeomorphic to a surface with handles. The number of handles is the \emph{genus}\index{genus} of the surface.
...
\begin{remark}
  The topological torus admits infinitely many nonisomorphic conformal structures. See example sheet 2. For future reference, the collection for a fixed surface of the conformal structures it admits is known as the \emph{Teichmüller space}\index{Teichmüller space}. It is the key object in the advanced study of Riemann surfaces.
\end{remark}
...
\begin{theorem}[uniformisation theorem]\index{uniformisation theorem}
...
\item \(X\) has \(\mathbb D\) as universal cover: we can only barely scratch the surface the final, and most interesting family. Recall (or note) that
  ...
  \]
  The subgroups of \(\PSL_2(\R)\) which act properly discontinuously are \emph{Fuschsian groups}\index{Fuschsian group}, studied in \emph{hyperbolic geometry}.
\end{enumerate}
...
\begin{corollary}[Picard]\index{Picard theorem}
...
\begin{corollary}[Riemann mapping theorem]\index{Riemann mapping theorem}
...
\[
  2g_X - 2 = 2(-2) + (2g + 2)
\]
so \(g_X = g\). This is a natural generalisation of Fermat curve\index{Fermat curve} and in particular shows that we can construct a Rieamnn surface with arbitrary genus.
