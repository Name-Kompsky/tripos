...
\begin{definition}[measure-preserving]\index{measure-preserving}\index{measure-preserving dynamical sysmtem}
  \(T: X \to X\) is \emph{measure-preserving} if \(T_*\mu = \mu\). \((X, \mathcal A, \mu, T)\) is called a \emph{measure-preserving dynamical sysmte}.
\end{definition}
...
\begin{theorem}[von Neumann mean ergodic theorem]\index{von Neumann mean ergodic theorem}
  Let \((X, \mathcal A, \mu, T)\) be a measure-preserving sysmte. Let \(f \in L^2(X, \mathcal A, \mu)\). Then the ergodic average
  \[
