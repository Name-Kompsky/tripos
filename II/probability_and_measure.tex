...
\begin{remark}
  ...
  \]
  so \(E^c\) is a \(\mu\)-null set. Replace each \(f_n\) and \(g\) by \(\mathbf 1_E f_n\). Then assumptions hold everywhere as \(\mu(f \mathbf 1_E) = \mu(f)\) for all \(f\) measurable.
\end{remark}
...
\begin{eg}\leavevmode
  ...
  \item exponential distribution of rate \(\lambda\):
    \begin{align*}
      ...
      \end{align*}
    \item Gaussian distribution with standart deviation \(\sigma\) and mean \(m\):
      \begin{align*}
...
\begin{eg}\leavevmode
  \begin{enumerate}
  \item Let \(\mu_n = \delta_{1/n}\) be the Dirac mass on \(\R^d\), where for \(x \in \R^d\), \(\delta_x\) is the probability on \(\R^d\) such that
    \[
      \delta_x(A) =
      \begin{cases}
        1 & x \in A \\
  ...
  \item Let \(\mu_n = \mathcal N(0, \sigma_n^2)\), Gaussian distribution with standard deviation \(\sigma_n\), where \(\sigma_n \to 0\), then again \(\mu_n \to \delta_0\). Indeed,
    ...
    \end{align*}
    \(\sigma_n \to 0\) so \(f(\sigma_n x) \to f(0)\) so by dominated convergence theorem, \(\mu_n(f) \to f(0) = \delta_0(f)\).
  \end{enumerate}
\end{eg}
...
\begin{lemma}
  ...
  \item there exists a family \(\mathcal F\) affine functions (\(\ell(x) = ax + b\)) such that \(\varphi = \sup_{\ell \in \mathcal F} \ell\) on \(I\).
  \end{enumerate}
\end{lemma}
...
Jensen is for probability space only. The following applies to all measure spaces.
...
\begin{proposition}[Minkowski inquality]\index{Minkowski inequality}
  ...
  \[
    \norm f_p = \left( \int_X |p|^p d \mu \right)^{1/p}.
  \]
  Then
  \[
    \norm{f + g}_p \leq \norm f_p + \norm g_p.
...
\begin{definition}
  ...
  \item For \(p = \infty\),
    ...
    where
    \[
      \essup |f| = \inf \{t \geq 0: |f(x)| \leq t \text{ for \(\mu\)-almost every } x\}.
    \]
  \end{enumerate}
\end{definition}
...
Claim that this is an equvalence relation stable under addition and multiplication.
