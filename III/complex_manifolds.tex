...
\begin{theorem}[Kähler identities]\index{Kähler identites}\leavevmode
  \begin{enumerate}
  \item \([\conj \p^*, L] = i \p\).
  \item \([\p^*, L] = -i \conj \p\).
...
We wish to understand the Dolbeault cohomology groups\index{Dobeault cohomology} \(H^{p, q}_{\conj \p}(X)\), and how they compare with the sheaf cohomology \(H^k (X, \C)\) where \(p + q = k\). We begin by picking canonical representatives of cohomology.
...
\]
so we can define \emph{harmonic forms}\index{harmonic forms} on \(X\) with respect to any of the Laplacian
\[
...
Denote \(h^{p, q} = \dim H_{\conj \p}^{p, q}(X)\). This is finite as \(X\) is compact. The \emph{Hodge diamond}\index{Hodge diamnond} is the array
\[
