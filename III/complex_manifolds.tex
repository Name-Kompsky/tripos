...
\begin{notation}
  Notation: if \(\mathcal U = \{U_\alpha\}_\alpha\) is an open cover indexed by a subset of \(\N\) (or any ordered set), we write
  \[
...
\begin{eg}
  Let \(\mathcal U = \{U_0, U_1, U_2\}\), \(\sigma = \{\sigma_0, \sigma_1, \sigma_3\} \in C^0(\mathcal U, \mathcal F)\). Then \(\delta \sigma\) is given by
  \begin{align*}
    (\delta \sigma)_{01} &= (\sigma_0 - \sigma_1)|_{U_{01}} \\
    (\delta \sigma)_{02} &= (\sigma_0 - \sigma_2)_{U_{02}} \\
    (\delta \sigma)_{12} &= (\sigma_1 - \sigma_2)_{U_12}
  \end{align*}
  and thus
  \[
