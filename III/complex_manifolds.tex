...
Dolbeaut cohomology is the obstruction to holomorphic sections and has its origin in the Mittag-Leffler problem: let \(S\) be a Riemann surface, i.e.\ one dimensional complex manifold. A \emph{principal part} at \(x \in S\) is a Laurent series of the form
\[
  \sum_{k = 1}^n a_k z^{-k}
\]
...
\begin{eg}
  If \(p: Y \to X\) is a morphism, \(E \to X\) is a holomorphic vector bundle then one obtains the \emph{pullback bundle}\index{pullback bundle} \(p^*E \to Y\) by simply pulling back transition functions. We write \(E|_Y\) if \(Y \subseteq X\) for the pullback under the inclusion map.
