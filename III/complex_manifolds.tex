...
\begin{proof}
  Let \(\omega\) be a Kähler metric and \(\tau = \bigwedge^q \omega\). Then \(\d \tau = 0\) as \(\d \omega = 0\) so \([\tau] \in H^{2q}_{\text{dR}} (X; \R)\). Suppose \(\tau = \d \sigma\) where \(\sigma \in \mathcal A^{2q - 1}_\R(X)\). Then
  \[
    \int_X \omega^n
...
\end{proposition}
This is analogous to the Riemannian geometric statement that we can choose a normal coordinates with to a Riemannian metric of this form.
...
\begin{proof}
  ...
  \[
    ...
    + \sum_{j, k, \ell} b_{j, k, \ell} \d \conj z_\ell \w \d z_j \w \d \conj z_k.
  \]
  Thus
  \begin{align*}
    a_{jk\ell} &= a_{\ell kj} \\
  ...
  Now let
  \[
    \xi_k = z_k + \frac{1}{z} \sum a_{jk\ell} z_j z_\ell,
  \]
  a valid change of coordinates in a neighbourhood of \(x\). Then
  \begin{align*}
    \d \xi_k &= \d z_k + \frac{1}{2} \sum a_{jk\ell} (z_j \d z_\ell + z_\ell \d z_j) \\
    \d \conj \xi_k &= \d z_k + \frac{1}{2} \sum \conj a_{jk\ell} (\conj z_j \d \conj z_\ell + \conj z_\ell \d \conj z_j)
  \end{align*}
  Now we compute their wedge product
  \begin{align*}
    \d \xi_k \w d \conj \xi_k
    &= \sum \d z_k \w \d \conj z_k \\
    &+ \frac{1}{2} \sum \conj a_{jk\ell} (\conj z_j \d z_k \w \d \conj z_\ell + \conj z_\ell \d z_k \w \d \conj z_j) \\
    &+ \frac{1}{2} \sum a_{jk\ell} (z_j \d \conj z_\ell \w \d \conj z_k + z_\ell \d z_j \w \d \conj z_k)
      + O(|z|^2) \\
    &= \sum \d z_k \w \d \conj z_k \\
    &+ \sum a_{jk\ell} z_\ell \d z_j \w \d \conj z_k + \sum b_{jk\ell} \conj z_\ell \d z_k \w \d \conj z_j + O(|z|^2) \\
    &= \frac{2}{i} \omega + O(|z|^2)
  \end{align*}
\end{proof}

Thus any identity only involving the metric \(h\) and its first derivative, valid on \(\C^n\) with its usual Kähler metric is true on any Kähler manifold. We'll use this several times.

Hodge theory. Revision of differential geometry
...
\begin{align*}
  \p: \mathcal A_\C^{p, q} &\to \mathcal A^{p + 1, q} \\
  \conj \p: \mathcal A_\C^{p, q} &\to \mathcal A^{p, q + 1}
\end{align*}
We define
\begin{align*}
  \p^* &= - \star \p \star \\
...
The operators \(\p^*\) and \(\conj \p^*\) are adjoint to \(\p, \conj \p\) respectively. Recall
\[
  \ip{\alpha, \beta}_{L^2} = \int_X \alpha \w \beta.
\]

\begin{lemma}
  Suppose \(\alpha \in \mathcal A_\C^{p, q}, \beta \in \mathcal A_\C^{n - (p + 1), n - q}\) then
  \begin{align*}
    \ip{\p^* \alpha, \beta}_{L^2} &= \ip{\alpha, \p \beta} \\
    \ip{\conj \p^* \alpha, \beta}_{L^2} &= \ip{\alpha, \conj \p \beta}
  \end{align*}
\end{lemma}
