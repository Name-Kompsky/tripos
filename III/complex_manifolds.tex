...
\begin{definition}[refinement of cover]\index{refine}
...
\paragraph{Short exact sequence of sheaves}
...
\begin{theorem}[Dolbeault]\index{Dolbeault's theorem}
  ...
  \[
    H^{p, q}_{\conj \p}(X) \cong H^q(X, \Omega^p)
  \]
  where \(\Omega^p(U)\) is the set of \(\sigma \in \mathcal A_\C^{p, 0}(U)\) with \(\conj \p \sigma = 0\).
\end{theorem}
