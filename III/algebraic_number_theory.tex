...
\begin{eg}
  ...
  \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
  The slopes are \(0, 1, 2\) each with multiplicity \(1\). Thus \(f(X)\) splits into linear factors in \(\Q_2[X]\). If \(f(X)\) is irreducible then \(E = \Q[X]/(f(X))\) is a cubic extension of \(\Q\) in which \(2\) splits completely.
\end{eg}
...
\begin{definition}[upper ramification group]\index{upper ramification group}
  If \(v \in \R_{\geq 0}\), we define the \emph{\(v\)th upper ramification group} to be \(G^v = G_{\varphi_{E/K}(v)} \leq G = \gal(E/K)\).
