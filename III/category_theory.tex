...
\begin{proof}
  ...
  \[
    \begin{tikzcd}
      P \ar[r, shift left] \ar[r, shift right] & P
    \end{tikzcd}
  \]
...
\begin{eg}\leavevmode
  \begin{enumerate}
  \item % from Mac Lane. An experiment: suppose you are not given the free group functor. How are you going to construct the left adjoint of forgetful functor?
    Consider the forgetful functor \(U: \c{Gp} \to \Set\). By 4.6a \(U\) creates all small limits so \(\c{Gp}\) has them and \(U\) preserves them. \(\c{Gp}\) is locally smal, given a set \(A\), any \(f: A \to UG\) factors as
    \[
      A \to UG' \to UG
    \]
    where \(G'\) is the subgroup generated by \(\{f(x): x \in A\}\) and \(\operatorname{card} G' \leq \max\{\aleph_0, \operatorname{card} A\}\). Let \(B\) be a set of this cardinality. Consider all subsets \(B' \subseteq B\), all group structures on \(B'\) and all mappings \(A \to B'\). These give us a solution set at \(A\). % grudge against Mac Lane.
...
Special adjoint functor theorem to get rid of solution set condition on functors
...
\begin{definition}[well-powered]\index{category!well-powered}
  We say \(\c C\) is \emph{well-powered} if each \(A \in \ob \c C\) has a set of subojects \(\{A_i \mono A: i \in I\}\) such that every subobject of \(A\) is isomorphic to some \(A_i\).

  Dually if \(\c C^{\text{op}}\) is well-powered, we say \(\c C\) is \emph{well-copowered}. (not cowell-powered, as it implies badly powered)
\end{definition}

For example in \(\Set\) we can take inclusions \(\{A' \embed A: A ' \in PA\}\). This is also where the name ``well-powered'' comes from as ``power set exists''
...
\begin{proof}
  The only if is given by RAPL. For the other direction, for any \(A \in \ob \c C\), \((A \downarrow G)\) is locally complete by 4.10, locally small, and well-powered since the subobjects of \((B, f)\) in \((A \downarrow G)\) are just those subobjects \(B' \mono B\) in \(\c D\) for which \(f\) factors through \(GB' \mono GB\). Also if \(\{S_i: i \in I\}\) is a coseparating set for \(\c D\) then the set
  \[
  ...
  So we need to show that if \(\mathcal A\) is complete, locally small and well-powered and has a coseparating set \(\{S_i: i \in I\}\) then \(\mathcal A\) has an initial object. Form the product \(P = \prod_i \in S_i\). ``industrial-stength pullback'' Now consider the diagram
  ...
  % \[
  %   \begin{tikzcd}
  %     B \ar[r] \ar[dr, rightarrowtail] & A \ar[d, "h", rightarrowtail] \\
  %     & P \ar[r, "k"]
  %   \end{tikzcd}
  % \]
...
\begin{remark}\leavevmode
  ...
  Cech's construction of \(\beta\) is as follow: given \(X\), form
  \[
  ...
  Cech's proof that this works is essentially the same as SAFT.
...
Does every monad comes from an adjunction? The answer is yes and is given independently in 1965 by Eilenberg, Moore Kleisli. We will cover both.
...
\begin{definition}[Kleisli category]\index{Kleisli category}
  ...
  \[
    \begin{tikzcd}
      A \ar[r, "f"] & TB \ar[r, "T_g"] & TTC \ar[r, "TTh"] \ar[d, "\mu_C"] & TTTD \ar[r, "T\mu D"] \ar[d, "\mu_{TD}"] \arrow[mysymbol]{dr}[description]{(3)} & TTD \ar[d, "\mu_D"] \\
      & & TC \ar[r, "Th"] & TTD \ar[r, "\mu_D"] & TD
    \end{tikzcd}
  \]
  commutes: the upper way monad is \((hg) f\) and the lower is \(h(gf)\).

  The unit laws similarly follow from
  \[
    \begin{tikzcd}
      A \ar[r, "f"] & TB \ar[r, "T\eta_B"] \ar[dr, "1_{TB}"'] \arrow[mysymbol]{dr}[description]{(1)} & TTB \ar[d, "\mu_B"] \\
      & & TB
    \end{tikzcd}
    \quad
    \begin{tikzcd}
      A \ar[r, "f"] \ar[d, "\eta_A"] & TB \ar[dr, "1_{TB}"] \ar[d, "1_{TB}"] \arrow[mysymbol]{dr}[description]{(2)}  \\
